...
class SimpleCacheTrunk(ComicTrunk):
    ...
    def __init__(self, cv_api_key: str, cache_expiry_minutes : int = 24*60, cache_filename: str | None = None,
        ...
        if cache_filename is None:
            self._cache_filename = 'requests-cache.sqlite'
        else:
        ...
        self._cache = SQLiteBackend(cache_name=self._cache_filename, expire_after=self._cache_expiry, autoclose=False)
    ...
    async def _process_response(self, endpoint: str, params: CommonParams, response_collection: type[SingleResponse | MultiResponse], \
            ...
            async with CachedSession(cache=self._cache) as session:
                response = await session.get(f'{self.cv_api_url}{endpoint}',
            ...
            if response.status != 200:  # noqa: PLR2004
                if response.status in self.response_map:
                    raise self.response_map[response.status]
    ...
    async def search(self, params: SearchParams) -> SearchResponse:
        ...
        async with CachedSession(cache=self._cache) as session:
            response = await session.get(f'{self.cv_api_url}/search',
                params=modified_params.model_dump(exclude_none=True),
                headers=self.headers)
